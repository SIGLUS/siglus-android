...
Then(/^I enter quantity number "(\d+)"$/) do |number|
  q = query("android.widget.EditText id:'et_movement_quantity'").first
  touch(q)
  keyboard_enter_text(number)
  hide_soft_keyboard
end
...
Then(/^I select movement reason$/) do
  query_string = "android.widget.EditText id:'et_movement_reason'"
  wait_for(:timeout => 2) {element_exists(query_string)}
  q = query(query_string).first
  touch(q)
end
...
Then(/^I select a new movement reason "(.*?)" "(.*?)"$/) do |first_reason, second_reason|
  steps %Q{
        Then I press the movement reason "#{first_reason}"
        Then I select movement reason
        Then I press the movement reason "#{second_reason}"
    	}
end

Then(/^I press the movement reason "(.*?)"$/) do |reason|
  query_string = "* id:'text' text:'#{reason}'"
  wait_for(:timeout => 2) {element_exists(query_string)}
  movement_reason = query(query_string).first
  touch(movement_reason)
end
...
Then(/^I make a new movement "(.*?)" "(.*?)" "(.*?)" "(.*?)"$/) do |stock_card_code, first_reason, second_reason, number|
  steps %Q{
        ...
        When I enter quantity "#{number}" for the new lot
        Then I wait for "Complete" to appear
        And I press "Complete"
        Then I navigate back
        Then I navigate back
