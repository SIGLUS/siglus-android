...
public class AddNonBasicProductsAdapter extends RecyclerView.Adapter<NonBasicProductViewHolder>{
    ...
    public void filter(final String keyword) {
        ...
        if (TextUtils.isEmpty(keyword)) {
            ...
            filteredViewModels = from(models).filter(new Predicate<NonBasicProductsViewModel>() {
                @Override
                public boolean apply(NonBasicProductsViewModel nonBasicProductsViewModel) {
                    return nonBasicProductsViewModel.getProduct().getProductFullName().toLowerCase().contains(keyword.toLowerCase());
                }
            }).toList();
        }
