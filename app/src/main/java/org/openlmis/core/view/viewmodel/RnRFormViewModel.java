...
public class RnRFormViewModel {
  ...
  private String generatePeriod(Date begin, Date end) {
    return LMISApp.getContext().getString(R.string.label_period_date, DateUtil.formatDate(begin),
        DateUtil.formatDate(end));
  }
  ...
  private void setName(String programCode) {
    switch (programCode) {
      ...
      case Program.MMTB_CODE:
        this.name = LMISApp.getContext().getString(R.string.tb_program_name);
        break;
      default:
  ...
  public static RnRFormViewModel buildNormalRnrViewModel(RnRForm form) {
    ...
    rnRFormViewModel.syncedTime = form.getSubmittedTime() == null ? StringUtils.EMPTY
        : DateUtil.formatDate(form.getSubmittedTime());
    rnRFormViewModel.title = rnRFormViewModel
        .generatePeriod(form.getPeriodBegin(), form.getPeriodEnd());
  ...
  public static RnRFormViewModel buildEmergencyViewModel(RnRForm form) {
    RnRFormViewModel rnRFormViewModel = new RnRFormViewModel();
    rnRFormViewModel.setViewModelFormField(form);
    rnRFormViewModel.title = LMISApp.getContext().getString(R.string.label_emergency_date,
        DateUtil.formatDate(form.getSubmittedTime(), DateUtil.TIME_FORMAT_WITHOUT_SECOND));
    rnRFormViewModel.syncedTime = DateUtil
        .formatDate(form.getSubmittedTime(), DateUtil.TIME_FORMAT_WITHOUT_SECOND);
    rnRFormViewModel.name = LMISApp.getContext()
        .getString(R.string.label_emergency_requisition_balance);
    rnRFormViewModel.setType(form);
    return rnRFormViewModel;
  }
  ...
  public static RnRFormViewModel buildMissedPeriod(Date startDate, Date endDate) {
    ...
    rnRFormViewModel.title = LMISApp.getContext()
        .getString(R.string.label_period_date, DateUtil.formatDateWithoutDay(startDate),
            DateUtil.formatDateWithoutDay(endDate));
    return rnRFormViewModel;
  }

  public static RnRFormViewModel buildFirstMissedPeriod(String programCode, Date startDate,
      Date endDate) {
    RnRFormViewModel rnRFormViewModel = new RnRFormViewModel();
    ...
    rnRFormViewModel.periodEndMonth = new DateTime(endDate);
    rnRFormViewModel.title = LMISApp.getContext()
        .getString(R.string.label_period_date, DateUtil.formatDateWithoutYear(startDate),
            DateUtil.formatDateWithoutDay(endDate));
    return rnRFormViewModel;
  }
