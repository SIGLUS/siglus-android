...
public class ALRequisitionPresenter extends BaseRequisitionPresenter {
  ...
  public void setViewModels() throws LMISException {
    ...
    for (ALGridViewModel gridViewModel : alReportViewModel.getItemTotal()
        .getAlGridViewModelList()) {
  ...
  private void addTreatment(List<RegimenItem> regimenItems, ALGridViewModel gridViewModel,
      String columnName) throws LMISException {
    RegimenItem itemTreatment = getRegimenItem(COLUMN_CODE_PREFIX_TREATMENTS + columnName,
        getRegimenType(columnName));
    itemTreatment.setHf(
        alReportViewModel.getItemHF().getAlGridViewModelMap().get(columnName).getTreatmentsValue());
    itemTreatment.setChw(alReportViewModel.getItemCHW().getAlGridViewModelMap().get(columnName)
        .getTreatmentsValue());
    itemTreatment.setAmount(gridViewModel.getTreatmentsValue());
    regimenItems.add(itemTreatment);
  }

  private RegimenItem getRegimenItem(String name, Regimen.RegimeType category)
  ...
  private void addStock(List<RegimenItem> regimenItems, ALGridViewModel gridViewModel,
      String columnName) throws LMISException {
    RegimenItem itemStock = getRegimenItem(COLUMN_CODE_PREFIX_STOCK + columnName,
        getRegimenType(columnName));
    itemStock.setHf(alReportViewModel.getItemHF().getAlGridViewModelMap().get(columnName)
        .getExistentStockValue());
    itemStock.setChw(alReportViewModel.getItemCHW().getAlGridViewModelMap().get(columnName)
        .getExistentStockValue());
