...
public class StockCard extends BaseModel implements Comparable<StockCard> {
  ...
  public boolean isOverStock() {
    return product.isHiv() ? stockOnHand > (int) Math.ceil(this.avgMonthlyConsumption * 3)
        : stockOnHand > (int) Math.ceil(this.avgMonthlyConsumption * 2);
  }
  ...
  public long calculateSOHFromLots(@NonNull List<LotOnHand> lotsOnHands) {
    long totalSOHFromLots = 0L;

    for (LotOnHand lotOnHand : lotsOnHands) {
      totalSOHFromLots += lotOnHand.getQuantityOnHand();
    }
  ...
  public Date getEarliestLotExpiryDate() {
    List<LotOnHand> lotOnHandList = FluentIterable.from(getNonEmptyLotOnHandList())
        .filter(lotOnHand -> lotOnHand.getQuantityOnHand() > 0).toList();
    if (lotOnHandList.isEmpty()) {
      return null;
    }
    return Collections.min(lotOnHandList,
        (lhs, rhs) -> lhs.getLot().getExpirationDate().compareTo(rhs.getLot().getExpirationDate()))
  ...
  public Date getLastStockMovementDate() {
    ...
    return Collections.max(getStockMovementItemsWrapper(),
        (lhs, rhs) -> lhs.getMovementDate().compareTo(rhs.getMovementDate())).getMovementDate();
  }
  ...
  public Date getLastStockMovementCreatedTime() {
    ...
    return Collections.max(getStockMovementItemsWrapper(),
        (lhs, rhs) -> lhs.getCreatedTime().compareTo(rhs.getCreatedTime())).getCreatedTime();
  }
