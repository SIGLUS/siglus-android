...
public class MovementReasonManagerTest {
  ...
  public void setUp() throws Exception {
    reasonManager = RoboGuice.getInjector(RuntimeEnvironment.application)
        .getInstance(MovementReasonManager.class);
  ...
  public void shouldNotDisplayPhysicalInventoryAndDefaultReasonOnMenu()
    ...
    assertThat(
        reasonManager.queryByCode(MovementReasonManager.INVENTORY).canBeDisplayOnMovementMenu(),
        is(false));
    assertThat(
        reasonManager.queryByCode(MovementReasonManager.DEFAULT_ISSUE).canBeDisplayOnMovementMenu(),
        is(false));
    assertThat(reasonManager.queryByCode(MovementReasonManager.DEFAULT_RECEIVE)
        .canBeDisplayOnMovementMenu(), is(false));
    assertThat(reasonManager.queryByCode(MovementReasonManager.DEFAULT_NEGATIVE_ADJUSTMENT)
        .canBeDisplayOnMovementMenu(), is(false));
    assertThat(reasonManager.queryByCode(MovementReasonManager.DEFAULT_POSITIVE_ADJUSTMENT)
        .canBeDisplayOnMovementMenu(), is(false));
    assertThat(
        reasonManager.queryByCode(MovementReasonManager.DONATION).canBeDisplayOnMovementMenu(),
        is(false));
    assertThat(
        reasonManager.queryByCode(MovementReasonManager.UNPACK_KIT).canBeDisplayOnMovementMenu(),
        is(false));
  }
