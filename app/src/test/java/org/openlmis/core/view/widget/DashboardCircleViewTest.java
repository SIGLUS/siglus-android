...
public class DashboardCircleViewTest {
  ...
  private void assertResult(ArrayList<DashboardCircleView.Item> items) {
    int totalAmount = 0;
    final float spaceTotalAngle =
        DashboardCircleView.ONE_PIECE_ANGLE * dashboardCircleView.getSpaceCount(items);
    ...
    final float anglePercent = new BigDecimal(item1.sweepAngle)
        .divide(new BigDecimal(360 - spaceTotalAngle), 2, BigDecimal.ROUND_HALF_UP).floatValue();
    final float amountPercent = new BigDecimal(Math.max(item1.amount, 0))
        .divide(new BigDecimal(totalAmount), 2, BigDecimal.ROUND_HALF_UP).floatValue();
  ...
  public void drawArcShouldInvokeCorrectTimes() {
    ...
    // then
    Mockito.verify(mockCanvas, Mockito.times(normalItems.size()))
        .drawArc(eq(dashboardCircleView.arcRectF), anyFloat(), anyFloat(), eq(false),
            eq(dashboardCircleView.circlePaint));
  }
}