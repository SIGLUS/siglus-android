...
import static org.openlmis.core.view.widget.SyncTimeView.GONE;
import static org.openlmis.core.view.widget.SyncTimeView.VISIBLE;
...
public class SyncTimeViewTest {
  ...
  public void shouldDisplayGreenIconAndTimeUnitIsMinute() throws Exception {
    // given
    when(sharedPreferenceMgr.getRnrLastSyncTime())
        .thenReturn(nowDateTime.minusMinutes(1).getMillis());
    when(sharedPreferenceMgr.getStockLastSyncTime())
        .thenReturn(nowDateTime.minusMinutes(2).getMillis());
    // when
    syncTimeView.showLastSyncTime();
    // then
    assertThat(sharedPreferenceMgr.getStockLastSyncTime(),
        is(nowDateTime.minusMinutes(2).getMillis()));
    assertThat(sharedPreferenceMgr.getRnrLastSyncTime(),
        is(nowDateTime.minusMinutes(1).getMillis()));
    assertThat(syncTimeView.txSyncTime.getText().toString(), is("1 minute since last sync"));
    assertThat(shadowOf(syncTimeView.ivSyncTimeIcon.getDrawable()).getCreatedFromResId(),
  ...
  public void shouldDisplayGreenIconAndTimeUnitIsHour() throws Exception {
    // given
    when(sharedPreferenceMgr.getRnrLastSyncTime())
        .thenReturn(nowDateTime.minusHours(2).getMillis());
    when(sharedPreferenceMgr.getStockLastSyncTime())
        .thenReturn(nowDateTime.minusHours(1).getMillis());
    // when
    syncTimeView.showLastSyncTime();
    // then

    assertThat(sharedPreferenceMgr.getStockLastSyncTime(),
        is(nowDateTime.minusHours(1).getMillis()));
  ...
  public void shouldDisplayYellowIconAndTimeUnitIsDay() throws Exception {
    ...
    when(sharedPreferenceMgr.getStockLastSyncTime())
        .thenReturn(nowDateTime.minusDays(2).getMillis());
    // when
    syncTimeView.showLastSyncTime();
    // then
    assertThat(sharedPreferenceMgr.getStockLastSyncTime(),
        is(nowDateTime.minusDays(2).getMillis()));
  ...
  public void shouldShowProgressBarWhenSyncStockCardLastYear() {
    ...
    assertThat(syncTimeView.txSyncTime.getText().toString(),
        is(LMISTestApp.getContext().getResources().getString(R.string.last_year_stock_cards_sync)));
  }
