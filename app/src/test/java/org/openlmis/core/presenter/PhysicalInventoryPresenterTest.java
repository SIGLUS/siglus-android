...
public class PhysicalInventoryPresenterTest extends LMISRepositoryUnitTest {
  ...
  public void setup() throws Exception {
    ...
    presenter = RoboGuice.getInjector(RuntimeEnvironment.application)
        .getInstance(PhysicalInventoryPresenter.class);
  ...
  @Test
  public void shouldLoadPhysicalStockCardListWithoutDeactivatedOrKitProducts()
      throws LMISException {
    StockCard stockCardVIA = StockCardBuilder.buildStockCard();
    stockCardVIA.setProduct(
        new ProductBuilder().setPrimaryName("VIA Product").setIsArchived(false).setIsActive(true)
            .build());

    StockCard stockCardMMIA = StockCardBuilder.buildStockCard();
    stockCardMMIA.setProduct(
        new ProductBuilder().setPrimaryName("MMIA Product").setIsArchived(false).setIsActive(false)
            .build());

    StockCard kitStockCard = StockCardBuilder.buildStockCard();
    kitStockCard.setProduct(
        new ProductBuilder().setPrimaryName("VIA Kit Product").setIsArchived(false)
            .setIsActive(true).setIsKit(true).build());
  ...
  public void shouldMakePositiveAdjustment() throws LMISException {
    ...
    model.getNewLotMovementViewModelList().add(
        new LotMovementViewModelBuilder().setExpiryDate("Jan 2015").setQuantity("120").build());
  ...
  @Test
  public void shouldMakeNegativeAdjustment() {
    InventoryViewModel model = new InventoryViewModel(stockCardWithIdNine);
    model.getNewLotMovementViewModelList()
        .add(new LotMovementViewModelBuilder().setExpiryDate("Jan 2015").setQuantity("80").build());
    stockCardWithIdNine.setStockOnHand(model.getLotListQuantityTotalAmount());
  ...
  @Test
  public void shouldCalculateStockAdjustment() {
    InventoryViewModel model = new InventoryViewModel(stockCardWithIdNine);
    model.getNewLotMovementViewModelList().add(
        new LotMovementViewModelBuilder().setExpiryDate("Jan 2015").setQuantity("100").build());
...
//        stockCard.setId(9);
    ...
    DraftLotItem draftLotItem1 = new DraftLotItem(lotMovementViewModel1,
        stockCardWithIdNine.getProduct(), false);
    DraftLotItem draftLotItem2 = new DraftLotItem(lotMovementViewModel2,
        stockCardWithIdNine.getProduct(), true);
    ...
    assertThat(inventoryViewModels.get(0).getNewLotMovementViewModelList().get(0).getLotNumber(),
        is("testNew"));
    assertThat(
  ...
  private InventoryViewModel buildInventoryViewModelWithOutDraft(int stockCardId, String quantity,
      String expireDate) {
    InventoryViewModel inventoryViewModelWithOutDraft = new PhysicalInventoryViewModel(
        buildDefaultStockCard(6));
  ...
  public void shouldPreventInversionDateOrTimeInsert() throws Exception {
    ...
    assertThat(subscriber.getOnErrorEvents().get(0).getMessage(),
        is(LMISTestApp.getContext().getResources().getString(R.string.msg_invalid_stock_movement)));
  }
  ...
  public void shouldGetCompleteCount() throws Exception {
    PhysicalInventoryViewModel inventoryViewModel = new PhysicalInventoryViewModel(
        stockCardWithIdThree);
