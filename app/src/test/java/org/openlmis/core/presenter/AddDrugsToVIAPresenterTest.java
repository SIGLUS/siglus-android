...
public class AddDrugsToVIAPresenterTest {
  ...
  public void loadActiveProductsNotInVIAForm() throws Exception {
    ...
    when(productRepository.queryActiveProductsInVIAProgramButNotInDraftVIAForm())
        .thenReturn(newArrayList(product1, product2, product3, product4));
  ...
  public void shouldConvertViewModelsToRnrFormList() throws Exception {
    ...
    when(productRepository.getByCode("P1")).thenReturn(
        new ProductBuilder().setCode("P1").setIsActive(true).setIsArchived(false).build());
    when(productRepository.getByCode("P2")).thenReturn(
        new ProductBuilder().setCode("P2").setIsActive(true).setIsArchived(true).build());

    presenter.getInventoryViewModelList()
        .addAll(newArrayList(inventoryViewModel1, inventoryViewModel2));
  ...
  private InventoryViewModel buildInventoryViewModel(String productCode, String quantity) {
    Product product1 = new ProductBuilder().setCode(productCode).setPrimaryName("product 1")
        .setIsActive(true).build();
    AddDrugsToViaInventoryViewModel inventoryViewModel1 = new AddDrugsToViaInventoryViewModel(
        product1);
