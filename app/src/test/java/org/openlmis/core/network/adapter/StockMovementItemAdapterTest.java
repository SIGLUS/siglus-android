...
public class StockMovementItemAdapterTest {
  ...
  public void shouldCreateStockMovementFromJSON() throws Exception {
    ...
    StockMovementItem stockMovementItem = stockMovementItemAdapter
    ...
    String movementDateString = new SimpleDateFormat(DateUtil.DB_DATE_FORMAT)
        .format(stockMovementItem.getMovementDate());
    assertEquals("2015-10-10", movementDateString);
    assertThat(stockMovementItem.getMovementType())
        .isEqualTo(MovementReasonManager.MovementType.NEGATIVE_ADJUST);
  ...
  public void shouldCreateStockMovementWithLotMovementsFromJSON() throws Exception {
    ...
    Assert
        .assertThat(stockMovementItem.getLotMovementItemListWrapper().get(0).getMovementQuantity(),
            is(-5L));
    Assert.assertThat(stockMovementItem.getLotMovementItemListWrapper().get(0).getStockOnHand(),
        is(5L));
    Assert.assertThat(
        stockMovementItem.getLotMovementItemListWrapper().get(0).getLot().getLotNumber(),
        is("TEST-A"));
  }
}